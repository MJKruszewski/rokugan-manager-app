            ...
            >
              ...
              <v-card-text v-if="$store.state.player !== undefined">
                <v-data-table
                    ...
                    :dark="this.$store.state.colorVariant"
                    :items="this.$store.state.player.equipment.filter(item => item.kind === 'weapon')"
                >
                  <template v-slot:[`item.name`]="{ item }">
                    {{ item.name }} ({{ item.grip }})
                  </template>
                  <template v-slot:[`item.damage`]="{ item }">
                    <v-chip :color="getDamageColorScale(item.damage)" :dark="$store.state.colorVariant">
                      {{ item.damage }}
                    </v-chip>
                  </template>
                  <template v-slot:[`item.deadlines`]="{ item }">
                    <v-chip :color="getDamageColorScale(item.deadlines)" :dark="$store.state.colorVariant">
                      {{ item.deadlines }}
                    </v-chip>
                  </template>
                  <template v-slot:[`item.qualities`]="{ item }">
                    <v-chip @click="openQualityBook(quality)" v-bind:key="quality + uuid.v4()" v-for="quality in item.qualities.split(' ').filter(val => val.length > 0)" :color="getQualityColor(quality)" :dark="$store.state.colorVariant">
                      {{ quality }}
                    </v-chip>
                  </template>
            ...
            >
              ...
              <v-card-text v-if="$store.state.player !== undefined">
                <v-data-table
                    ...
                    hide-default-footer
                    single-select
                    :headers="armorsHeaders"
                    :dark="this.$store.state.colorVariant"
                    :items="this.$store.state.player.equipment.filter(item => item.kind === 'armor')"
                >
                  <template v-slot:[`item.physRes`]="{ item }">
                    <v-chip :color="getArmorColorScale(item.physRes)" :dark="$store.state.colorVariant">
                      {{ item.physRes }}
                    </v-chip>
                  </template>
                  <template v-slot:[`item.superRes`]="{ item }">
                    <v-chip :color="getArmorColorScale(item.superRes)" :dark="$store.state.colorVariant">
                      {{ item.superRes }}
                    </v-chip>
                  </template>
                  <template v-slot:[`item.qualities`]="{ item }">
            ...
            >
              ...
              <v-card-text v-if="$store.state.player !== undefined">
                <v-data-table
                    ...
                    :dark="this.$store.state.colorVariant"
                    :items="this.$store.state.player.equipment.filter(item => item.kind === 'other')"
                >
                  <template v-slot:[`item.qualities`]="{ item }">
                    <v-chip @click="openQualityBook(quality)" v-bind:key="quality + uuid.v4()" v-for="quality in item.qualities.split(' ').filter(val => val.length > 0)" :color="getQualityColor(quality)" :dark="$store.state.colorVariant">
                      {{ quality }}
