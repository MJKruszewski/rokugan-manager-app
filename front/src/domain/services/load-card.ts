...
import {Armor, Book, Equipment, Player, Weapon} from "@/domain/types/player.type";
...
export class LoadCard {
    ...
    static async importCard(xml: Document): Promise<Player> {
        ...
        LoadCard.prepareInventory(xml, placeholder);
        LoadCard.prepareAbilities(xml, placeholder);
        LoadCard.preparePersonalTraits(xml, placeholder);
